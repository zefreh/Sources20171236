...
Wisconsin2 = function(ConfigList, st){
  ConfigList = SetState(ConfigList, st, "Wsc2") 
  
  set.seed(0)
  
  old <- Sys.time() # get start time
  data = read.table("D:/RProjects/OptTask/Data/fernandezMain/imbalanced/wisconsin/wisconsin.dat", 
                    sep=",", 
                    fill=FALSE, 
                    strip.white=TRUE)
