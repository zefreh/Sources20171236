...
australian  = function(ConfigList){
  ConfigList[1,"DBName"] = "Ast"
  
  set.seed(0)
  old <- Sys.time() # get start time
  #  Read and Correct data
  data = read.table("D:/RProjects/OptTask/Data/australian.txt", 
                    sep=" ", 
                    fill=FALSE, 
                    strip.white=TRUE)
  data$Sequence <- seq.int(nrow(data))
  data = data[,c(15,16,1:14)]
  ConfigList$CategryFields="3,6,7,8,10,11,13,14"
