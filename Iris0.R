...
Iris0 = function(ConfigList, st){
  ConfigList = SetState(ConfigList, st, "Irs0") 
  
  set.seed(0)
  
  old <- Sys.time() # get start time
  data = read.table("D:/RProjects/OptTask/Data/fernandezMain/imbalanced/iris0/iris0.dat", 
                    sep=",", 
                    fill=FALSE, 
                    strip.white=TRUE)
  data$Sequence <- seq.int(nrow(data))
  data$ClassLabel <- 0
  data[which(data$V5 == "positive"), "ClassLabel"] = 1
