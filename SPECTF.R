...
SPECTF  = function(ConfigList, st){
  ConfigList = SetState(ConfigList, st, "Spc")

  set.seed(0)
  old <- Sys.time() # get start time
  #  Read and Correct data
  data = read.table("D:/RProjects/OptTask/Data/SPECTF1.txt", 
                    sep=",", 
                    fill=FALSE, 
                    strip.white=TRUE)
  ...
  for (foldVar in 1:10){
    ...
    for (sp in 1:ConfigList$spCounts){
      ...
      trainData = DescritizeFeature(trainData, 37)
      FtrCtgList = GetAllCategoryList(ConfigList, FtrCtgList, trainData, 37, sp)
      trainData = DescritizeFeature(trainData, 38)
      FtrCtgList = GetAllCategoryList(ConfigList, FtrCtgList, trainData, 38, sp)
      trainData = DescritizeFeature(trainData, 39)
      FtrCtgList = GetAllCategoryList(ConfigList, FtrCtgList, trainData, 39, sp)
      trainData = DescritizeFeature(trainData, 40)
      FtrCtgList = GetAllCategoryList(ConfigList, FtrCtgList, trainData, 40, sp)
      trainData = DescritizeFeature(trainData, 41)
      FtrCtgList = GetAllCategoryList(ConfigList, FtrCtgList, trainData, 41, sp)
      trainData = DescritizeFeature(trainData, 42)
      FtrCtgList = GetAllCategoryList(ConfigList, FtrCtgList, trainData, 42, sp)
      trainData = DescritizeFeature(trainData, 43)
      FtrCtgList = GetAllCategoryList(ConfigList, FtrCtgList, trainData, 43, sp)
      trainData = DescritizeFeature(trainData, 44)
      FtrCtgList = GetAllCategoryList(ConfigList, FtrCtgList, trainData, 44, sp)
      trainData = DescritizeFeature(trainData, 45)
      FtrCtgList = GetAllCategoryList(ConfigList, FtrCtgList, trainData, 45, sp)
      trainData = DescritizeFeature(trainData, 46)
      FtrCtgList = GetAllCategoryList(ConfigList, FtrCtgList, trainData, 46, sp)
