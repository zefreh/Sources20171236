...
vehicle0 = function(ConfigList, st){
  ConfigList = SetState(ConfigList, st, "vh0") 
  
  set.seed(0)
  
  old <- Sys.time() # get start time
  data = read.table("D:/RProjects/OptTask/Data/fernandezMain/imbalanced/vehicle0/vehicle0_5.dat", 
                    sep=",", 
                    fill=FALSE, 
                    strip.white=TRUE)
  data$Sequence <- seq.int(nrow(data))
  data$ClassLabel <- 0
  data[which(data$V19 == "positive"), "ClassLabel"] = 1

  data = data[,c(21,20,1:18)]
