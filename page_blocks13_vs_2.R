...
page_blocks13_vs_2 = function(ConfigList, st){
  ConfigList = SetState(ConfigList, st, "PB13vs2") 
  
  set.seed(0)
  
  old <- Sys.time() # get start time
  data = read.table("D:/RProjects/OptTask/Data/fernandezMain/imbalanced/page_blocks13_vs_2/page_blocks13_vs_2.dat", 
                    sep=",", 
                    fill=FALSE, 
                    strip.white=TRUE)
  data$Sequence <- seq.int(nrow(data))
  data$ClassLabel <- 0
  data[which(data$V11 == "positive"), "ClassLabel"] = 1
  
  data = data[,c(13,12,1:10)]
