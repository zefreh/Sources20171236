...
Chronic  = function(ConfigList){
  ConfigList[1,"DBName"] = "Chr"
  set.seed(0)
  old <- Sys.time() # get start time
  #  Read and Correct data
  data = read.table("D:/RProjects/OptTask/Data/Chronic.txt", 
                    sep=",", 
                    fill=FALSE, 
                    strip.white=TRUE)
  data$Sequence <- seq.int(nrow(data))
  data$ClassLabel <- -1
  ind <- data$V25 %in% "notckd"     
  data[ind, "ClassLabel"] <- 0   
  ind <- data$V25 %in% "ckd"     
  data[ind, "ClassLabel"] <- 1   
  
  data = data[,c(27,26,1:24)]
  data$V3 = (data$V3 * 1000 - 1000) / 5  
  ConfigList$CategryFields="5,6,7,8,9,10,11,21,22,23,24,25,26"
  ...
  data$V20 <- as.integer(data$V20)
  data$V21 <- as.integer(data$V21)
  data$V22 <- as.integer(data$V22)
  data$V23 <- as.integer(data$V23)
  data$V24 <- as.integer(data$V24)
  ...
  for (foldVar in 1:10){
    ...
    for (sp in 1:ConfigList$spCounts){
      ...
      trainData[,"LowB"] = trainData[,21]
      trainData[,"UprB"] = trainData[,21]
      ...
      trainData[,"LowB"] = trainData[,23]
      trainData[,"UprB"] = trainData[,23]
      ...
      trainData[,"LowB"] = trainData[,25]
      trainData[,"UprB"] = trainData[,25]
