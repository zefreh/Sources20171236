...
Haberman  = function(ConfigList, st){
  ConfigList = SetState(ConfigList, st, "Hbr")
  set.seed(0)
  old <- Sys.time() # get start time
  #  Read and Correct data
  data = read.table("D:/RProjects/OptTask/Data/Haberman.txt", 
                    sep=",", 
                    fill=FALSE, 
                    strip.white=TRUE)
  ...
  positiveClass = 1
  crossValidCounts=10
  folds = createFolds(label,k=crossValidCounts)
  TestAUCList = InitializeAlg(ConfigList, data)
  AllWList = GetAllWList()
  
  foldVar = 1
  for (foldVar in 1:crossValidCounts){
