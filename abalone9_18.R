...
abalone9_18 = function(ConfigList, st){
  ConfigList = SetState(ConfigList, st, "ab9_18")
  set.seed(0)
  
  old <- Sys.time() # get start time
  data = read.table("D:/RProjects/OptTask/Data/fernandezMain/imbalanced/abalone9_18/abalone9_18_5.dat", 
                    sep=",", 
                    fill=FALSE, 
                    strip.white=TRUE)
  data$Sequence <- seq.int(nrow(data))
  data$ClassLabel <- 0
  data[which(data$V9 == "positive"), "ClassLabel"] = 1

  data = data[,c(11,10,1:8)]
  ConfigList$CategryFields="3"
  ...
  for (foldVar in 1:10){
    ...
    for (sp in 1:ConfigList$spCounts){
      ...
      for (j in 3:ncol(trainData))
      {
        ...
        {
          trainData[,"LowB"] = trainData[,j]
          trainData[,"UprB"] = trainData[,j]
          FtrCtgList = GetAllCategoryList(ConfigList, FtrCtgList, trainData, j, sp)
        }
        else
        {
          trainData = DescritizeFeature(trainData, j)
          FtrCtgList = GetAllCategoryList(ConfigList, FtrCtgList, trainData, j, sp)
        }
      }
