...
vowel0 = function(ConfigList, st){
  ConfigList = SetState(ConfigList, st, "VW0") 
  
  set.seed(0)
  
  old <- Sys.time() # get start time
  data = read.table("D:/RProjects/OptTask/Data/fernandezMain/imbalanced/vowel0/vowel0.dat", 
                    sep=",", 
                    fill=FALSE, 
                    strip.white=TRUE)
  data$Sequence <- seq.int(nrow(data))
  data$ClassLabel <- 0
  data[which(data$V14 == "positive"), "ClassLabel"] = 1

  data = data[,c(16,15,1:13)]
