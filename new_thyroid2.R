...
new_thyroid2 = function(ConfigList, st){
  ConfigList = SetState(ConfigList, st, "NTr2") 
  
  set.seed(0)
  
  old <- Sys.time() # get start time
  data = read.table("D:/RProjects/OptTask/Data/fernandezMain/imbalanced/new_thyroid2/new_thyroid2.dat", 
                    sep=",", 
                    fill=FALSE, 
                    strip.white=TRUE)
  data$Sequence <- seq.int(nrow(data))
  data$ClassLabel <- 0
  data[which(data$V6 == "positive"), "ClassLabel"] = 1
