...
segment0 = function(ConfigList, st){
  ConfigList = SetState(ConfigList, st, "sgm") 
  
  set.seed(0)
  
  old <- Sys.time() # get start time
  data = read.table("D:/RProjects/OptTask/Data/fernandezMain/imbalanced/segment0/segment0.dat", 
                    sep=",", 
                    fill=FALSE, 
                    strip.white=TRUE)
  data$Sequence <- seq.int(nrow(data))
  data$ClassLabel <- 0
  data[which(data$V20 == "positive"), "ClassLabel"] = 1
  
  data = data[,c(22,21,1:19)]
