...
Glass0123_456 = function(ConfigList, st){
  ConfigList = SetState(ConfigList, st, "Gls0123_456") 
  
  set.seed(0)
  
  old <- Sys.time() # get start time
  data = read.table("D:/RProjects/OptTask/Data/fernandezMain/imbalanced/glass0123_vs_456/glass0123_vs_456.dat", 
                    sep=",", 
                    fill=FALSE, 
                    strip.white=TRUE)
  data$Sequence <- seq.int(nrow(data))
  data$ClassLabel <- 0
  data[which(data$V10 == "positive"), "ClassLabel"] = 1
  
  data = data[,c(12,11,1:9)]
