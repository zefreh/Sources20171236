...
Haberman2 = function(ConfigList, st){
  ConfigList = SetState(ConfigList, st, "Hbr2") 
  
  set.seed(0)
  
  old <- Sys.time() # get start time
  data = read.table("D:/RProjects/OptTask/Data/fernandezMain/imbalanced/haberman/haberman_5.dat", 
                    sep=",", 
                    fill=FALSE, 
                    strip.white=TRUE)
  data$Sequence <- seq.int(nrow(data))
  data$ClassLabel <- 0
  data[which(data$V4 == "positive"), "ClassLabel"] = 1
